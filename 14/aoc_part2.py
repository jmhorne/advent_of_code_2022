...
class Cave:
    def __init__(self, cave_data) -> None:
        ...
        self.cave = np.full((self.y_max, self.x_max), ".", dtype=str)
        self.full = False
        ...
        for line in cave_data:
            for x in range(len(line) - 1):
                self.draw_rock(line[x], line[x + 1])
        
        self.cave[self.y_max - 1] = "#"
    ...
    def find_size(self, cave_data):
        ...
        for line in cave_data:
            for point in line:
                xs.append(point[0])
                ys.append(point[1])
        return min(xs), max(xs) + 1, max(ys) + 3
    ...
    def drop_sand(self):
        ...
        try:
            while not sand_at_rest:
                if falling_sand_pos[1] + 1 >= len(self.cave[0]):
                    self.add_col()
            ...
            if self.cave[self.sand_start] == "o":
    ...
    def add_col(self):
        col = np.full((self.y_max, 1), ".", dtype=str)
        col[self.y_max - 1, 0] = '#'
        self.cave = np.hstack((self.cave, col))
